...
static mpq_srcptr
mpq_roinit_normal_nn(mpq_t x, mp_srcptr np, mp_size_t ns, mp_srcptr dp, mp_size_t ds)
{
...
{
	return mpq_roinit_normal_nn(x, n->_mp_d, n->_mp_size, d->_mp_d, d->_mp_size);
}
...
{
	return (mpz_cmp(mpq_numref(a), mpq_numref(b)) == 0) && (mpz_cmp(mpq_denref(a), mpq_denref(b)) == 0);
}
...
mpq_cmp_si(const mpq_t q, signed long n, unsigned long d)
{
	...
	else
	{
		...
		else
		{
			...
			mpq_roinit_normal_nn(
				t,
				mpq_numref(q)->_mp_d,
				-mpq_numref(q)->_mp_size,
				mpq_denref(q)->_mp_d,
				mpq_denref(q)->_mp_size);
...
mpq_sub(mpq_t r, const mpq_t a, const mpq_t b)
{
	...
	mpq_roinit_normal_nn(
		t,
		mpq_numref(b)->_mp_d,
		-mpq_numref(b)->_mp_size,
		mpq_denref(b)->_mp_d,
		mpq_denref(b)->_mp_size);
	mpq_add(r, a, t);
}
